%% calculate new CSM from fastMRI image using ESPIRiT
function [csm, reference] = cal_csm(img_coil, full_flag)
addpath ./function/ESPIRiT_code
coil_num = size(img_coil, 3);
...
csm = M(:,:,:,end).*repmat(W(:,:,end)>eigThresh_2,[1,1,coil_num]);
end