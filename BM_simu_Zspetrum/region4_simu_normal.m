%% simulation of normal tissue (GM/WM/CSF) using B-M equation
...
for n_T1w=1:length(T1w_range_d)-1
    ...
    for n_T2w=1:length(T2w_range_d)-1
        ...
        for n_Mapt=1:length(Mapt_range_d)-1
            ...
            eval(['z_normal.z_',num2str(n_Mapt),'=','zSpectrum',';']);
        end
        save([save_dir,filesep,'z_normal_T1_',num2str(n_T1w),'_T2_',num2str(n_T2w),'.mat'],'z_normal');
        num_all = num_all+1;
        disp(['*** num of z-spectra : ',num2str(num_all*(length(Mapt_range_d)-1)*(length(Mmt_range_d)-1)*z_num)])
    end
end
%% show Z & MTRa
% Z=squeeze(z_normal.z_1(1,1,2:length(scanned_offset_sorted)+1));
% freq=scanned_offset_sorted/123.47;
% min(Z)
% MTRa=Z(length(Z):-1:floor(length(Z)/2)+2)-Z(1:floor(length(Z)/2));
% figure(),plot(freq, Z, '-o')
% hold on, Z=squeeze(z_skin.z_1(1,1,2:length(scanned_offset_sorted)+1));
% plot(freq, Z, '-o'), set(gca,'XDir','reverse')
% ylim=get(gca,'Ylim'); hold on;plot([3.5,3.5],ylim,'m--');hold on;plot([-3.5,-3.5],ylim,'m--');
% legend('WM','skin','Location','southeast')
