%% simulation of edema tissue using B-M equation
...
for n_T1w=1:length(T1w_range_d)-1
    ...
    for n_T2w=1:length(T2w_range_d)-1
        ...
        for n_Mapt=1:length(Mapt_range_d)-1
            ...
            eval(['z_edema.z_',num2str(n_Mapt),'=','zSpectrum',';']);
        end
        save([save_dir,filesep,'z_edema_T1_',num2str(n_T1w),'_T2_',num2str(n_T2w),'.mat'],'z_edema');
        num_all = num_all+1;
        disp(['*** num of z-spectra : ',num2str(num_all*(length(Mapt_range_d)-1)*(length(Mmt_range_d)-1)*z_num)])
    end
end
%% show Z & MTRa
% Z=squeeze(z_edema.z_5(1,1,2:length(scanned_offset_sorted)+1));
