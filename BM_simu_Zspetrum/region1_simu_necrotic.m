%% simulation of necrotic tissue using B-M equation
% 2023.06.05 by Wang Yuyan
clear, close all
%% input params
save_dir = fullfile('..','Data','simuZ_t');
...
Mapt_range_d=Mapt_range(1)+(0:0.5:1).*(Mapt_range(2)-Mapt_range(1));
...
for n_T1w=1:length(T1w_range_d)-1
    ...
    for n_T2w=1:length(T2w_range_d)-1
        ...
        for n_Mapt=1:length(Mapt_range_d)-1
            ...
            eval(['z_necro.z_',num2str(n_Mapt),'=','zSpectrum',';']);
        end
        save([save_dir,filesep,'z_necro_T1_',num2str(n_T1w),'_T2_',num2str(n_T2w),'.mat'],'z_necro');
        num_all = num_all+1;
        disp(['*** num of z-spectra : ',num2str(num_all*(length(Mapt_range_d)-1)*(length(Mmt_range_d)-1)*z_num)])
    end
end
%% show Z & MTRa
% Z=squeeze(z_necro.z_1(1,1,2:length(scanned_offset_sorted)+1));
